...
class Trainer():
    ...
    def load_config(self):
        ...
        self.project = self.yaml_file["train_img"]
    ...
    def set_unique_paths(self, train_split, unique_imgs):
        ...
        if self.active_learning:
            ...
            val_fold_size = int(len(unique_imgs)*(1-self.train_split))
    ...
    def set_paths(self, cross_val_run=0, train=False, test=False, pred=False):
        ...
        if train:
            ...
            self.exp_dir = Path(os.path.join(self.yaml_file["exp_dir"], "exp_{0}_{1}_s{2}".format(self.project, self.encoder, self.seed)))
            if self.cross_validation:
                self.exp_dir = Path(str(self.exp_dir) + "_cv{0}".format(self.cross_val_run))
                if self.cross_val_run == 0:
                    self.cross_val_exp_series = str(uuid.uuid4())[:6]
                    exp_dir = Path(str(self.exp_dir) + "_" + str(self.cross_val_exp_series))
                else:
                    exp_dir = Path(str(self.exp_dir) + "_" + str(self.cross_val_exp_series))
    ...
    def create_dataloaders(self, augmentations=True):
        ...
        self.train_loader = DataLoader(self.train_dataset, batch_size=self.train_batch_size, shuffle=True, num_workers=0)
        self.valid_loader = DataLoader(self.valid_dataset, batch_size=self.val_batch_size, shuffle=False, num_workers=0)
    ...
    def prepare_model(self):
        ...
        original_batch_update = self.train_epoch.batch_update

        def amp_batch_update(x, y):
            self.optimizer.zero_grad()
            with torch.cuda.amp.autocast():
                prediction, loss = original_batch_update(x, y)
            self.scaler.scale(loss).backward()
            self.scaler.step(self.optimizer)
            self.scaler.update()
            return prediction, loss

        self.train_epoch.batch_update = amp_batch_update
    ...
    def train_model(self, epochs=1, al_step=25, dry_run=False):
        ...
        if not dry_run:
            self.run_train_loop(epochs=epochs, al_step=al_step)
            self.save_loss_plots()
        elif dry_run:
            print("Dry run for testing...")
            self.dry_run_loop(epochs=epochs)
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        print("Training mode...")
        trainer = Trainer(active_learning=False, save_val_uncertainty=False, cross_validation=True, config_file=config_file) # create Trainer object, load config & set default values
        for encoder in encoder_list:
            trainer.set_encoder(encoder) # set encoder for model
            for cross_val_run in range(cross_val_runs):
                print("Train: ", encoder, "\n")
                ...
                trainer.train_model(epochs=300) # start training routine using Training/Validation Epoch Runners
                # trainer.test_model()
